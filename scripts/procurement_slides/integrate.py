...
def status():
    ...
    stats = [("25 類", "主副採已盤點的工作類型（14＋11）", NAVY),
             ("10 位", "人力編制（主採 7＋副採 3）｜27R0 預算", NAVY),
             ("12.6%", "主採 AI 可協助比例（占比加權）；副採待評估", BLUE),
             ("9 項", "主採已導入 5 支 skill ＋ 副採潛在可介入點 4 項", BLUE)]
    gap = 0.24
    sw = (CONTENT_W - gap * 3) / 4
    for i, (num, lab, col) in enumerate(stats):
        x0 = MARGIN_X + i * (sw + gap)
        s += shape(x0, 1.68, sw, 0.98, PANEL, "roundRect")
        s += shape(x0, 1.68, 0.06, 0.98, col)
        s += tb(x0 + 0.24, 1.77, sw - 0.44, 0.40, [[run(num, 2100, 1, col)]])
        s += tb(x0 + 0.24, 2.19, sw - 0.44, 0.38, [[run(lab, 900, 0, BODY)]], lnspc=122)

    lx, lw = MARGIN_X, 5.85
    rx, rw = MARGIN_X + 6.24, 5.85
    panel_y, panel_h, note_y = 3.08, 2.66, 4.98

    s += tb(lx, 2.76, lw, 0.26, [[run("主採｜已導入 5 支 skill", 1250, 1, NAVY)]])
    ...
    s += tb(rx, 2.76, rw, 0.26, [[run("副採｜4 個潛在可介入點", 1250, 1, BLUE)]])
    ...
    s += tb(rx + 0.26, note_y, rw - 0.52, 0.62,
            [[run("分布在合計約 40% 的類別中（20＋5＋10＋5）；可自動化的只是這些類別裡的"
                  "登錄與計算步驟，不是整類工作。副採尚未做同口徑的加權估算。", 900, 0, BODY)]], lnspc=128)

    s += shape(MARGIN_X, 5.92, CONTENT_W, 0.86, REDBG, "roundRect")
    s += shape(MARGIN_X, 5.92, 0.06, 0.86, RED)
    s += tb(MARGIN_X + 0.24, 6.02, 2.30, 0.24, [[run("⚠ 這兩個數字的定義", 1050, 1, RED)]])
...
def why():
    ...
    cards = [
        ...
        ("② 等待不會因為 AI 而變快",
         "工時花在等供應商、成衣廠、客戶回覆。就算報表 3 秒生成，對方明天才回 —— 處理速度從來不是瓶頸。"),
        ("③ 要看實物才能拍板 —— 靠專業與經驗",
         "色差、移染、白汙、看色核色與手感，都要目視、觸摸實物並與標準樣比對；還要送樣、複測、與外部 KAKEN 討論報告，打樣後再複核看色。"),
        ("④ 溝通載體不在系統裡",
         "缸差核色靠微信、重大異常靠電話與現場會議；下班後與假日（工廠仍在上班）也要即時回應工廠、廠商、客人的對話與圖片 —— AI 讀不到，也回不了。"),
        ("⑤ 承諾與責任要人來扛",
         "下採購單即產生商業約束力；異常究責與 CAP 報告都要有人對客人、工廠、廠商說明。責任無法交給工具。"),
        ("⑥ 資料會回頭改寫上一輪的答案",
         "訂單分批下單、分割生產與分廠重算、TA 變動、PH3 填補 —— 前一刻算出的答案，隨時被工廠或供應商的回覆推翻。"),
    ]
    cols, gap = 3, 0.24
    ...
    s += tb(MARGIN_X + 0.32, 4.60, CONTENT_W - 0.64, 0.58,
            [[run("主採最大的一類占 15%、副採最大的兩類各占 20% —— 沒有一塊大到值得整批自動化；"
                  "而省下的 10–15% 散在十個人每天的 10–30 分鐘切片裡，", 1200, 1, WHITE),
              run("屬於等待中的零碎空隙，不易整併為可釋出的編制。", 1200, 1, "FFD9D9")]],
            anchor="ctr", lnspc=128)

    advice = [
        ("① 繼續擴大 AI 輸入，人力投入有價值產出", NAVY,
         "擴大結構化比對類工具的覆蓋；目標是把這 10–15% 的行政步驟做穩、降低 key-in 錯誤，將人的工時花在預防問題與提高對外溝通效能。"),
        ("② 這幾類不納入 AI 取代規劃", RED,
         "重大品質異常、議價協商（含匯率商談）、看色與核色判斷、品質預防、跨廠催辦、跨國交涉 —— 維持現有人力配置。"),
        ("③ 評估員額編制看量體", NAVY,
         "日後若要評估人力，以款數／PO 張數／異常件數等量體指標判斷。"),
    ]
    y = 5.40
    ...
    s += source2("結論：AI 定位為協作工具，主要效益在支撐產能而非釋出編制。｜"
